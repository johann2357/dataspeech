...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_file", default=None, type=str, help="File to process")
    parser.add_argument("--data_file_pattern", default=None, type=str, help="File to process")
    ...
    if args.data_file_pattern:
        files = [f for f in os.listdir(args.dataset_name) if fnmatch.fnmatch(f, args.data_file_pattern)]
    elif args.data_file:
    ...
    for file in files:
        ...
        for split in dataset.keys():
            if dataset[split]:
                ...
                text_description = [
                    "A male quechua speaker from central Peru delivers his speech in a somewhat quiet and confined space, with a mildly monotone tone, a slightly low pitch, and a slow pace. The recording appears to be of good quality with only a slight background noise.",
                ] * len(dataset[split]["speaker_id"])
                dataset[split] = dataset[split].add_column("text_description", text_description)
                df = dataset[split].to_pandas()
                df.to_parquet(f"output/{file}", engine='pyarrow')
